...
let money=10000, chip=10, bets={P:0,B:0,T:0}, lastBet={P:0,B:0,T:0}, shoe=[], hist=[], busy=false, round=1, timer=15, profit=0;
function save(){localStorage.macauV71=JSON.stringify({money,bets,lastBet,shoe,hist,round,profit})}
function load(){try{let s=JSON.parse(localStorage.macauV71||'{}'); if(s.money!=null){money=s.money;bets=s.bets||bets;lastBet=s.lastBet||lastBet;shoe=s.shoe||[];hist=s.hist||[];round=s.round||1;profit=s.profit||0}}catch(e){} if(shoe.length<80)newShoe(); render(); setInterval(()=>{if(!busy){timer--; if(timer<0)timer=15; $('timer').textContent=timer}},1000)}
...
function render(){
 const totalBet=bets.P+bets.B+bets.T, denom=Math.max(1,totalBet);
 $('money').textContent=money.toLocaleString('zh-CN'); $('betP').textContent=bets.P; $('betB').textContent=bets.B; $('betT').textContent=bets.T;
 $('pctP').textContent=Math.round(bets.P/denom*100)+'%'; $('pctB').textContent=Math.round(bets.B/denom*100)+'%'; $('pctT').textContent=Math.round(bets.T/denom*100)+'%';
 $('roundNo').textContent=String(round).padStart(6,'0'); $('shoeLeft').textContent='牌靴 '+shoe.length; $('profit').textContent=profit;
...
$('doubleBtn').onclick=()=>{if(busy)return; let sum=bets.P+bets.B+bets.T; if(!sum)return toast('请先下注'); if(money<sum)return toast('余额不足'); money-=sum; bets.P*=2; bets.B*=2; bets.T*=2; render(); save()};
$('dealBtn').onclick=async()=>{
 ...
 money+=payout; let delta=payout-stake; profit+=delta; hist.push({res,pt,bt,delta}); if(hist.length>150)hist.shift();
 toast((res==='P'?'闲赢':res==='B'?'庄赢':'和局')+' '+(delta>=0?'+':'')+delta);
